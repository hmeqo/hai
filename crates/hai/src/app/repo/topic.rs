...
impl TopicRepo {
    ...
    pub async fn create(
        ...
        let row = sqlx::query_as!(
            Topic,
            r#"
            INSERT INTO topic (chat_id, title, meta, started_at, last_active_at)
    ...
    /// 更新话题最后活跃时间
    pub async fn touch(pool: &PgPool, id: Uuid) -> Result<u64> {
        let result = sqlx::query!(
            "UPDATE topic SET last_active_at = CURRENT_TIMESTAMP WHERE id = $1",
            id,
        )
        .execute(pool)
        .await?;
