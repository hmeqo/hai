...
use teloxide::{
    ...
    prelude::*,
    types::{FileId, Me, MessageEntityKind, ReplyParameters},
...
use crate::{
    agent::AgentHandler,
    agent::event::{AgentEvent, GroupTrigger, TriggerCause, TriggerStatus},
    config::AppConfig,
    domain::{
        entity::{Account, Chat, ChatType, Platform},
        service::Services,
        vo::{
            ImageOptions, MessageMeta, PlatformAccountMeta, PlatformMessageMeta,
            TelegramAccountMeta, TelegramContentPart, TelegramMessageMeta,
        },
    },
};
...
impl BotHandler {
    ...
    fn dispatch_agent_event(&self, chat_id: i64, chat_type: ChatType, msg: &Message, me: &Me) {
        ...
        if let Some(reason) = self.group_trigger.on_message(chat_id, is_mention)
        ...
        match cmd {
            ...
            Command::AvailableModels => {
                ...
                bot.send_message(
                    chat_id,
                    format!(
                        "当前 provider: {} (type: {})\n已配置 providers: {}\n\n参考模型列表: https://openrouter.ai/models",
            ...
            Command::OrganizeMemory => {
                let inner_chat_id = bh.get_internal_chat_id(&msg).await?;
                bh.agent_event_tx.send(AgentEvent::Message {
                    chat_id: inner_chat_id,
                    cause: TriggerCause::Command(
                        "静默执行记忆整理, 包括不限于处理不符合规范的记忆, 删除重建".into(),
                    ),
                })?;
            }
            Command::Profile => {
                let personality = bh.agent_handler.personality.config();

                let dims_text: Vec<String> = personality
                    .dims()
                    .iter()
                    .map(|(name, val, scale)| format!("  {}: {:.2} ({})", name, val, scale))
                    .collect();

                let interests = if personality.interests.is_empty() {
                    "无".into()
                } else {
    ...
    async fn extract_reply_media(reply: &Message, bot: &Bot) -> Result<Option<(String, String)>> {
        if let Some(text) = reply.text() {
            ...
            let url = get_file_url(bot, file_id.clone()).await?;
    ...
    async fn handle_image(
        ...
        if let Some(reply) = msg.reply_to_message()
            && let Some((url, _)) = Self::extract_reply_media(reply, bot).await?
        {
            image_url = Some(url);
        }
    ...
    async fn handle_to_text(
        ...
        if let Some(reply) = msg.reply_to_message() {
            ...
            let audio_data = get_file_content(bot, file_id.clone()).await?;
            let resp = bh
                .agent_handler
...
async fn get_file_url(bot: &teloxide::Bot, file_id: FileId) -> Result<String> {
...
async fn get_file_content(bot: &teloxide::Bot, file_id: FileId) -> Result<Vec<u8>> {
    let file = bot.get_file(file_id).await?;
