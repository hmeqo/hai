...
impl MessageRepo {
    ...
    /// 批量标记消息状态
    async fn mark_status(pool: &PgPool, message_ids: &[i64], status: &'static str) -> Result<u64> {
        let result = sqlx::query!(
    ...
    pub async fn mark_replied(pool: &PgPool, message_ids: &[i64]) -> Result<u64> {
        let status: &'static str = MessageStatus::Replied.into();
        Self::mark_status(pool, message_ids, status).await
    }

    /// 标记指定消息中未标记的为已阅（自动过滤已标记的）
    pub async fn mark_unread_seen(pool: &PgPool, message_ids: &[i64]) -> Result<u64> {
    ...
    /// 通过内部消息 ID 查找消息
    pub async fn find_by_id(pool: &PgPool, id: i64) -> Result<Option<Message>> {
        let row = sqlx::query_as!(
            Message,
            r#"
            SELECT
    ...
    /// 批量按 ID 查找消息
    pub async fn find_by_ids(pool: &PgPool, ids: &[i64]) -> Result<Vec<Message>> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
